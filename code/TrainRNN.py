...
Trains an RNN model for character-level language modeling using modules from module.py.
Follows the structure of TrainGPT.py.
"""

import torch
import torch.nn.functional as F
from module import Embedding, RNN, Dropout, Linear, Sequential, LayerNorm
...
def load_training_data():
    ...
    return """
    Hello world! This is a simple example text for training an RNN model.
...
class RNNModel:
    def __init__(self, vocab_size, n_embd, n_hidden, block_size, dropout, num_layers=2, device='cpu'):
        ...
        for i in range(num_layers):
            ...
            self.rnn_layers.append(RNN(input_size, n_hidden, device=device))
    ...
    def parameters(self):
        ...
        for r, n in zip(self.rnn_layers, self.norm_layers):
            params += r.parameters() + n.parameters()
    ...
    def __call__(self, idx, targets=None):
        ...
        for rnn, norm in zip(self.rnn_layers, self.norm_layers):
            out = rnn(x)  # (B, T, n_hidden)
...
model = RNNModel(vocab_size, n_embd, n_hidden, block_size, dropout, num_layers=num_layers, device=device)
